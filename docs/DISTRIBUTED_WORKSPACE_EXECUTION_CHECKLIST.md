...
### PR-S1 (ZiggySpiderweb): `control.workspace_status` desired/actual/drift payload
...
### PR-C1 (ZiggyStarSpider): parse + render desired/actual/drift
...
### PR-S2 (ZiggySpiderweb): background reconcile loop
...
### PR-S3 (ZiggySpiderweb): event-driven reconcile triggers
...
### PR-S6 (ZiggySpiderweb): high-level project bootstrap control op
...
### PR-S8 (ZiggySpiderweb): scoped token policy + audit stream
- Separate operator/project/node scopes with validation.
- Emit structured audit records for mutating ops.

### PR-S9 (ZiggySpiderweb): correlation IDs end-to-end
...
### PR-C8 (ZiggyStarSpider): correlation visibility
